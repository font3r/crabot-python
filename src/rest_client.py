...
async def handle_command(session: aiohttp.ClientSession, msg: MessageEvent):
	if msg.content == "zjeb":
		await send_message(session, msg.channel_id, "<@!269132306227265536> Sebastian Winek")
	else:
...
async def send_message(session: aiohttp.ClientSession, channel: str, messsage: str):
	response = await session.post(f"{DISCORD_API_GATEWAY}/channels/{channel}/messages", 
		headers={
